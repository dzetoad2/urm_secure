	...
	while($row = mysql_fetch_array($result))
	{
		...
		if(defined('DEBUG')){
		 ...
		 $o .=  '<tr class="customFacilityRow" id="'.$row['id'].'">'.$editCell.'<td class="nameCell" id="'.$row['name'].'">'.$row['name'].'</td><td>'.$row['address'].'</td><td>'.$row['city'].
  		           '</td><td>'.$row['state'].'</td><td>'.$row['zip'].'</td><td class="'.$customFacilityTypeIdClass.'" id="'.$row['facilityTypeId'].'"><a class="unclickable '.$linkTransparentClass.'" href="" >'.$title.'</td></tr>';
		}
	}
	return $o;
