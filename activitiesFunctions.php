...
//	 */
//	$result = mysql_query("  select  activity.id as id, activity.idNum as idNum, activity.title as title  from activity  where activityCategoryId = ".$activityCategoryId.";");           //check un/pw against db.
//	if($result === FALSE){
...
function getActivityRowsHtml($userId, $facilityId, $isCustomFacility, $isCustomActivity, $activityCategoryId){
	 ...
	 * select userFacility.id, facility.name from user
	 ...
	 where user.username = 'admin'
	 */
	$result = mysql_query("  select  id, idNum, title from activity where activityCategoryId = ".$activityCategoryId."   order by idNum;");           //check un/pw against db.
	if($result === FALSE){
		throwMyExc('getActivityRowsHtml: query failed');
	}
	$numrows = mysql_num_rows($result);
	if($numrows===false){
...
function deleteActivity($activityId){
	$res1 = mysql_query("  delete from activity where id = ".$activityId."");  
	$r = mysql_affected_rows();
	$res2 = mysql_query("  delete from surveyAnswer where userId = 1 and isCustomActivity=0 and activityId=".$activityId."");  
